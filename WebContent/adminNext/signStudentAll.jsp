							...
							<table class="table">
								...
								<tbody>
									...
									<tr>
										...
										<td><%=signup1.getStatus()%></td>

										<%
											ExamRoomDao exam_roomd = new ExamRoomDaoImpl();
													if (signup1.getRoom_id() != 0) {
														exam_room room1 = exam_roomd.getRoomInfo(signup1.getRoom_id());
													}
										%>

										<td><%=signup1.getRoom_id() == 0 ? "未分配" : signup1.getRoom_id()%></td>

										<td><%=signup1.getSeat_num() == 0 ? "未分配" : signup1.getSeat_num()%></td>

										<td>
											<form
												action="/GradeMan/arrangeExam_roomNext?user_id=<%=signup1.getUser_id()%>&exam_id=<%=exam_id%>"
												method="post">
												<%
													ExamRoomDao erd = new ExamRoomDaoImpl();
															List<exam_room> erooms = erd.getRoomInfobyEI(exam_id);
												%>
												<div class="col-xs-8">
												<select name="exam_room" class="form-control">
													<%
														for (exam_room er1 : erooms) {

																	SignUpDao supd = new SignUpDaoImpl();
																	List<signup> as = supd.getAllsignup();
																	int numr = 0;
																	for (signup si : as) {
																		if (er1.getRoom_id() == si.getRoom_id()) {
																			numr++;
																		}

																	}
																	int res = er1.getExam_room_num() - numr;
													%>
													<option value="<%=er1.getRoom_id()%>">考场名称：<%=er1.getRoom_name()%>剩余容量：<%=res%></option>
													<%
														}
													%>
												</select> 
												...
												<div class="col-xs-4">
												<input type="submit" class="btn btn-primary" value="分配考场">
												</div>
