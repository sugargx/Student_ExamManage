							...
							<table class="table">
								...
								<tbody>
									<%
									    ...
									    List<signup> students = signd.getSignupExam(exam_id);
									    for(signup s1:students){
									...
									<tr>
										...
										<%
											UserDao userd = new UserDaoImpl();
												user user1 = userd.getUserbyID(s1.getUser_id());
										%>

										<td><%=user1.getName()%></td>
										<td><%=user1.getIdentify() %></td>
										...
										%>
											<form
												action="/GradeMan/AuditGoNext?user_id=<%=s1.getUser_id()%>&exam_id=<%=exam_id%>"
												method="post">
												
												<input type="submit" class="btn btn-primary btn-xs"  style="display:<%=s1.getStatus().equals("通过") ? "none":" " %>"
													value="通过">

											</form>
											
											<form
												action="/GradeMan/AuditGoNoNext?user_id=<%=s1.getUser_id()%>&exam_id=<%=exam_id%>"
												method="post">
												
												<input type="submit" class="btn btn-primary btn-xs" style="display:<%=s1.getStatus().equals("不通过") ? "none":" " %>"
													value="不通过" onclick="return confirm('您确定要审核为不通过吗？')">
