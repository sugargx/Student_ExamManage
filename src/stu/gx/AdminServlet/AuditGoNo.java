...
/**
 * Servlet implementation class AuditGoNo
 */
public class AuditGoNo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       signup s = Sdao.getExamUser(user_id, exam_id);
     	...
     	Sdao.updateStatus("不通过", s.getExamcard_number());
     	
         //若原本状态为通过，若变为不通过，报名成功人数-1
     	 ExamDao examd = new ExamDaoImpl(); 
     	 if(s.getStatus().equals("通过")) {
...
//                         "审核结果为：" + "通过";
         ...
         String str = "同学您好，您的审核结果为：不通过！审核为不通过的原因可能为您填写的报名信息有误或由考试发布方导致！点我到网站(〃'▽'〃)";
         //设置编码，防止发送的内容中文乱码。
         try {
     	...
     	out.print("<script language='javascript'>alert('审核修改成功并成功给用户发送邮件通知审核结果！');window.location.href='/GradeMan/admin/signExamStudentAll.jsp?exam_id="
    			+ exam_id + "';</script>");
